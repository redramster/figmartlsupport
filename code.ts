...
async function main(): Promise<string | undefined> {
	...
	setInterval(function(){
		...
		if (figma.currentPage.selection[0]!=selection){
			...
			let tf:TextNode=<TextNode>selection
			figma.ui.postMessage({text:reverseIt(tf.characters,false),width:tf.width,height:tf.height,font:(<FontName>tf.fontName).family,fontSize:tf.fontSize,textAlign:tf.textAlignHorizontal.toLowerCase()})
			return
		}
...
figma.ui.onmessage = msg => {
  ...
  if (msg.type === 'cancel'){
	...
	if (selection.type=="TEXT"){
		let tf :TextNode= selection
		let fon :FontName =<FontName>tf.fontName
...
function wrapLines(val:string){
	/* make sure  all wrap generated lines */
	let results=""
	let textElm :TextNode = figma.createText()
	const tf: TextNode = <TextNode>figma.currentPage.selection[0]
	let fon :FontName =<FontName>tf.fontName
	//figma.loadFontAsync({ family: fon.family, style: fon.style}).then((res)=>{
		let curLine:string=""
		let curLineWidth:number=0
		for (let i=0;i<val.length;i++){
			let curLetter:string=val.substr(i,1)
			...
			if (curLineWidth>tf.width){
				...
				if (curLetter!=" "){
					for (let j:number=curLine.length-1;j>1;j--){
...
function shuffleLine (theLine:string, theShuffleChar:string) {
	//alert (theLine);
...
function reverseIt (val:string,wrap:boolean) {
